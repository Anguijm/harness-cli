...
**Vector blend on `harness recall` (priority #6).** `harness recall` blends keyword × recency with cosine similarity from a Gemini-embedding index when `.harness/embeddings.json` exists. Run `harness reindex` to build / refresh it (incremental by default — only changed chunks are re-embedded). The blend surfaces semantically-similar entries that share no literal tokens with the query; without the index, recall falls back silently to keyword-only behavior. `--no-vector` skips the blend per-call even when the index is present. **Persistent kill switch:** set `recall.vector: false` in `harness.yml` to disable the blend without per-call flags — short-circuits before the embed API call, so there's no cost even if an index exists. Stronger than deleting the cache (which `harness reindex` would silently regenerate). The index file is gitignored — it's a derivable cache, not load-bearing. Add `.harness/embeddings.json` to your repo's `.gitignore` if `harness init` didn't already.

**When lint surfaces a duplicate-signal cluster, run `harness synthesize` to draft the page.** The command finds clusters with ≥3 failures sharing a signature, calls Gemini, and appends a clearly-marked auto-draft section to `learnings.md` with an idempotency marker (`<!-- synthesis: <hash> -->`) so re-runs don't duplicate. The draft is **not load-bearing on its own** — review the section, refine the recommended fix, and confirm before treating it as the canonical synthesis. Defaults are read-only (dry-run lists clusters); `--apply` writes and requires `GEMINI_API_KEY`. Sanitization mirrors the council's `<UNTRUSTED_TOOL_OUTPUT>` discipline — failure records are wrapped as `<UNTRUSTED_FAILURE_RECORD>` with control-char strip and tag escape so a crafted failure entry can't manipulate the synthesizer.
