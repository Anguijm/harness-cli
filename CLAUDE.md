...
**Vector blend on `harness recall` (priority #6).** `harness recall` blends keyword × recency with cosine similarity from a Gemini-embedding index when `.harness/embeddings.json` exists. Run `harness reindex` to build / refresh it (incremental by default — only changed chunks are re-embedded). The blend surfaces semantically-similar entries that share no literal tokens with the query; without the index, recall falls back silently to keyword-only behavior. `--no-vector` skips the blend even when the index is present. The index file is gitignored — it's a derivable cache, not load-bearing. Add `.harness/embeddings.json` to your repo's `.gitignore` if `harness init` didn't already.
