...
Each persona file is a generic skeleton with a `{{PROJECT_NAME}}` token (substituted at `harness init` time). After init, **edit each persona's `## Scope` section** to describe the actual surface of this repo: what modules exist, what contracts matter, what the threat model is, what the cost drivers are. Generic personas produce generic critiques. Specialized personas catch real bugs.
